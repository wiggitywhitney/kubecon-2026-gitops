...
```{.yaml code-line-numbers="|2-3|4-6|7"}
connectors:
  spanmetrics:
    namespace: gen_ai
    dimensions:
...
::: {.notes}
Walk through the real YAML. First highlight: spanmetrics connector with namespace gen_ai (produces metric gen_ai_calls_total). Second highlight: dimensions — these are the promoted attributes from the transform step. Third highlight: flush every 15 seconds. The result is a counter metric with dimensions for service_name, score.label, and story.part.
:::
