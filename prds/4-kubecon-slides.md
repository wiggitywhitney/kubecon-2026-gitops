...
**Purpose**: Introduce OpenTelemetry, span events, and gen_ai semantic conventions. Assumed knowledge: KubeCon audience knows the name "OpenTelemetry" but not span events or semantic conventions.
**Content** (builds incrementally):
1. You tap thumbs up → app creates an OpenTelemetry span (`evaluate UserSatisfaction`) — introduce "span" casually as a unit of work
2. **What's a span event?** — its own beat. A span event is OTel's structured log, automatically correlated with the trace. It's a thing that happened *during* a span. Most people think OTel = spans and metrics; span events are the underused third concept.
3. The span event (`gen_ai.evaluation.result`) carries your vote — introduce **gen_ai semantic conventions** as a standard vocabulary (not custom fields) that any OTel tool can read. Attributes: `score.label` = thumbs_up, `score.value` = 1.0, `story.part` = 3
4. Linked back to generation + story part — the vote isn't floating in space; it's connected to the generation that produced the content
5. **"Here's what this looks like"** — link to Datadog APM trace view showing a real span event with its attributes. Quick aside, then move on.
**Key teaching points**: Span events (OTel's structured log). Gen_ai semantic conventions (standard vocabulary, not custom). Linking evaluation to generation. Concrete "see it in practice" via Datadog.
**Diagram**: LR flowchart building incrementally: You → App → span → span event → linked to generation span.

### Scene 4: The Collector — From Events to Metrics
**Purpose**: Show how the OTel Collector transforms raw telemetry into actionable metrics. Introduce the Collector concept first — the audience hasn't seen it explained yet.
**Content** (builds incrementally):
1. **What's an OTel Collector?** — its own intro slide. The Collector is a pipeline that receives, processes, and exports telemetry. It sits between your apps and your backends.
2. Traces arrive at Collector via OTLP
3. **Transform processor**: promotes span event attributes up to parent span (spanmetrics reads span attributes, not events — this is the non-obvious trick)
4. **Spanmetrics connector**: converts spans into counter metric `gen_ai_calls_total` with dimensions: `service_name`, `span_name`, `gen_ai.evaluation.score.label`, `story.part`
5. Two outputs fork: Prometheus exporter (in-cluster, for Flagger) and Datadog exporter (for dashboards)
...
**Diagram**: LR pipeline flow building incrementally — OTLP receiver → transform → spanmetrics → fork to Prometheus + Datadog.
...
- [ ] **M7: Polish and standalone readability** — Ensure slides are self-contained (readable without presenter). Add context text, refine speaker notes, verify all diagrams render correctly in Quarto Reveal.js. Final pass on `slides/TALK-FLOW.md` to ensure it matches the finished slides.
